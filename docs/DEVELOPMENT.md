...
- **Signing:** F-Droid signs with its own key, so its build and the GitHub or Play build can't update each other. Back up first when switching from one to the other. To get one signature everywhere instead, add `AllowedAPKSigningKeys` (the SHA-256 of our signing certificate) and `Binaries: https://github.com/tfthushaar/metakai/releases/download/v%v/metakai-%v-foss.apk` to the recipe: F-Droid then rebuilds each release and publishes the GitHub APK only if the bytes match. That has to be decided before the first F-Droid release, and hasn't been checked for this app.
